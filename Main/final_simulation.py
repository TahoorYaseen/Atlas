...
import sys
import os
my_dict={'1':[[[1,2,3],[1,1,3],[2,3,4],[2,2,4]],[2,2,2,2]]} #just an exmaple
# options={
...
def initialize():
    ts1 = TrafficSignal(0, amber_time, my_dict['1'][1][0], defaultMinimum, defaultMaximum)
    signals.append(ts1)
    ts2 = TrafficSignal(ts1.yellow+ts1.green+4, amber_time, my_dict['1'][1][1], defaultMinimum, defaultMaximum)
    signals.append(ts2)
    ts3 = TrafficSignal(ts2.red+ts2.green+ts2.yellow+4, amber_time, my_dict['1'][1][2], defaultMinimum, defaultMaximum)
    signals.append(ts3)
    ts4 = TrafficSignal(ts3.red+ts3.green+ts3.yellow+4, amber_time, my_dict['1'][1][3], defaultMinimum, defaultMaximum)
    signals.append(ts4)
    repeat()

# Set time according to formula
def setTime():
    global noOfCars, noOfBikes, noOfBuses, noOfLanes,vehicle_data
    global carTime, busTime, bikeTime,my_dict
    global currentGreen, currentYellow, nextGreen,ix,jx,allred,cycle_length
    vehicle_data=[]
    my_dict={}
    os.system("say detecting vehicles")
    noOfCars, noOfBuses, noOfBikes = 0,0,0
    for lane in range(4):
        for j in range(len(vehicles[directionNumbers[lane]][0])):
            vehicle = vehicles[directionNumbers[lane]][0][j]
            if(vehicle.crossed==0):
                vclass = vehicle.vehicleClass
                # print(vclass)
                noOfBikes += 1
        for i in range(1,3):
            for j in range(len(vehicles[directionNumbers[lane]][i])):
                vehicle = vehicles[directionNumbers[lane]][i][j]
                if(vehicle.crossed==0):
                    vclass = vehicle.vehicleClass
                    # print(vclass)
                    if(vclass=='car'):
                        ...
                        noOfBuses += 1
        vehicle_data.append([noOfBuses,noOfCars,noOfBikes])
    print(vehicle_data)
    all_green_time=third_dirty_dummy(vehicle_data)
    my_dict['1']=[vehicle_data,all_green_time]
    last_green=my_dict['1'][1][3]
    sum_green=sum(my_dict['1'][1])-last_green
    last_red_rime=18+sum_green
    cycle_length=last_green+last_red_rime+amber_time

    signals[currentGreen].green = my_dict['1'][1][currentGreen]
    signals[currentGreen].yellow = amber_time
    signals[currentGreen].red = cycle_length - (my_dict['1'][1][currentGreen]+amber_time)
    ...
    '''use your function to calculate the green signal time here'''
...
def third_dirty_dummy(vehicle_data):
    l, r = 2, 12
    n = 4
    SF_max=24
    SF_min=6
    ...
    q = [sum(vehicle_data[0]),sum(vehicle_data[1]),sum(vehicle_data[2]),sum(vehicle_data[3])]
#     print(q,"numberofvehicles")
    #optional code start
    min_lane=(min(eb,wb,nb,sb))
    eb_lane=eb/min_lane
    sb_lane=sb/min_lane
    wb_lane=wb/min_lane
    nb_lane=nb/min_lane
    
    y = np.array([q[0] / (((SF_max+SF_min)*eb_lane*n)), q[1] / (((SF_max+SF_min)*sb_lane*n)), q[2] / (((SF_max+SF_min)*wb_lane*n)), q[3] / (((SF_max+SF_min)*nb_lane*n))])
   
    Y = round(np.sum(y),6)

    Co = ((1.5 * L) + 5) / (1 - Y)
    rounded_Co = 5 * round(Co / 5)

    g_eb = round((y[0] / Y) * (rounded_Co - L))
    g_sb = round((y[1] / Y) * (rounded_Co - L))
    g_wb = round((y[2] / Y) * (rounded_Co - L))
    g_nb = round((y[3] / Y) * (rounded_Co - L))
    return [g_eb, g_sb, g_wb, g_nb]


def repeat():
   

    global currentGreen, currentYellow, nextGreen,jx,allred,cycle_length,my_dict
    amber_time=2
    allred=0

    if jx%4==0 and jx!=0:   # set time of next green signal 
